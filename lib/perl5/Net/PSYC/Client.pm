...
sub get_context {
	...
	$addr = $vars->{'_context'} || $vars->{'_source_relay'} 
		|| $vars->{'_source'};
    }
...
sub msg {
    ...
    W1('Client->msg(%s, %s, %.10s ..., %p)', $source, $mc, $data, $vars);
    my $func = $react{$mc};
    if ($func) {
	...
	my $time = int($vars->{'_time_place'}||$vars->{'_time_log'});
	if ($time + 5 < time()) {
...
sub sendmsg {
    ...
    $MMPvars ||= {};
    $MMPvars->{'_source_identification'} = UNI();
...
'_notice_link'		=> sub {
    my ($source, $mc, $data, $vars) = @_;
    # suboptimal, we parse even if the tag is given..
    my @user = parse_uniform($source);
    my @uni = parse_uniform(UNI());
    # allow psyc server to have a better name than we thought, or to
    # redirect us to a better identity.
    if ($$vars{'_tag_reply'} eq "perlpsyc$$" or
	    same_host($user[1], $uni[1]) && lc($user[4]) eq lc($uni[4])) {
	$UNI = $source;
	if (exists $vars->{'_nick'}) {
	    $NICK = $vars->{'_nick'};
	}
	Net::PSYC::get_connection($UNI)->TRUST(6);
	# this hack was specific to psyc developers who also use irc clients ;)
	# but no longer necessary since psyced will always set echo on for
	# psyc clients.
	#sendmsg(UNI(), '_request_execute', 'set echo on');
    } else {
	W0('Got a _notice_link from a UNI (%s) we did not try to link to.', 
	    $source);
    }
},
'_notice_unlink' => sub {
