...
sub ping_init {
    ...
    Net::PSYC::Event::remove($self->{'ping_id'}, 't') if exists $self->{'ping_id'};
    $self->{'ping_sub'} ||= sub { 
...
if (Net::PSYC::FORK) {
    ...
    $m = make_psyc('_notice_circuit_established', 
		      'Connection to [_source] established!');
    # peeruniform does not exist. it should contain the _source that sent us
    # the _notice_circuit_established, or just stay zero. sending $peeraddr
    # is incorrect as the server doesn't want to be addressed by any hostname
    # or ip it wasn't configured to handle.
    $self->fire($self->{'peeruniform'}, $m, $h);
...
sub send {
    ...
    if (!exists $vars->{"_source"}) {
	if ($PSYC::BASE) {
	    $vars->{"_source"} = $PSYC::BASE;
	} elsif (exists $self->{'me'}) {
