...
tie %MMPVARS, 'Net::PSYC::Tie::AbbrevHash';
# sync'd to "routing" in psyced's net/library/share.c
%MMPVARS = (
    '_amount_fragments'	=> 0,
    '_context'	=> 1,
    '_count'	=> 1,
    '_fragment'	=> 0,
    '_length'	=> 0,
    # AbbrevHash would handle this, but mentioning it is faster anyhow
    '_source_identification'	=> 1,
    '_source_relay'	=> 1,
    '_source'	=> 1,
    '_tag_reply'	=> 1,
    '_tag_relay'	=> 1,
...
our @EXPORT = qw(bind_uniform psyctext make_uniform UNL sendmsg parse_uniform);
...
sub SW {
	...
	$sec -= $^T;
	W(sprintf("%d.%d\t".$f, $sec, $msec, @_), $level);    
    } else {
	W(sprintf($f, @_), $level);    
    }
}
...
#   $MMPvars->{'_target_relay'} ||= $vars{'_target_relay'};
...
See http://www.psyc.eu for protocol specs and other info on PSYC.
...
allocate a context on a psyced and sendmsg to it.
...
renders the strings in B<$vars> into the B<$format> and returns the resulting text conformant to the text/psyc content type specification. compatible to psyced psyctext. 
...
Apart from the shortcuts below every single function may be exported seperately. You can switch on Eventing by using 
    ...
    # or
    use Net::PSYC qw(Event=Glib);
    # or
    use Net::PSYC qw(Event=Event); # Event.pm

Apart from IO::Select and Event the Glib eventing is quite limited. I does not offer support for timer-events.

=over 4

=item use Net::PSYC qw(:encrypt);

Try to use ssl for tcp connections. You need to have L<IO::Socket::SSL> installed. Right now only tls client functionality works. Works with psyced. 

=item use Net::PSYC qw(:compress);

Use L<Compress::Zlib> to compress data sent via tcp. Works fine with Net::PSYC and psyced.
...
exports bind_uniform, psyctext, make_uniform, UNL, sendmsg and  parse_uniform.
